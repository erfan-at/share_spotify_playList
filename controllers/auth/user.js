...
module.exports = {
    ...
    userSignup: async (req, res, next) => {
        try {
            let userExist = await User.findOne({ mobile: req.body.mobile, role: "user", softDelete: false })
            if (userExist) {
                const user = {
                    mobile: userExist.mobile ? userExist.mobile : undefined,
                    username: userExist.username == req.body.username ? userExist.username : undefined,
                    email: userExist.email == req.body.email ? userExist.email : undefined,
                }
                return resBuilder.conflict(res, user, '.کاربری با مشحصات وارده در سیستم وجود دارد ')
            }
            user = new User({
                name: req.body.name, mobile: req.body.mobile,
                password: cryptography.md5(req.body.password),
                email: req.body.email, username: req.body.username, role: "user"
            })
            await user.save();
    ...
    userEntrance: async (req, res, next) => {
        try {
            ...
            const user = await User.findOne({ mobile: req.body.mobile, role: "user", softDelete: false })
            if (user) {
                ...
                } else { return resBuilder.invalidReq(res, "", " شماره موبایل یا کد ارسالی اشتباه است") }
